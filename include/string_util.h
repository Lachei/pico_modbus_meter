...
inline std::string_view extract_line(std::string_view &content) {
	std::size_t whitespace = content.find_first_not_of(" \n\r");
	if (whitespace != std::string_view::npos)
		content = content.substr(whitespace);
	return extract_until_newline(content);
}
